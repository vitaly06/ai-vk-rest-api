...
func (h *UserHandler) handleMainState(ctx context.Context, u *models.User, msg object.MessagesMessage, cmd, text string) {
	...
	case "user_clear_map":
		h.clearUserDialogHistory(ctx, u, models.DialogMap, "Карта")
	case "support":
...
func (h *UserHandler) clearUserDialogHistory(ctx context.Context, u *models.User, dtype models.DialogType, title string) {
	...
	if err != nil {
		...
		h.base.send(ctx, u.VKID, "Не удалось открыть историю для очистки.", keyboards.MainMenu())
		return
	}
	if err := h.dialogRepo.ClearHistory(ctx, dialog.ID); err != nil {
		...
		h.base.send(ctx, u.VKID, "Не удалось очистить историю.", keyboards.MainMenu())
		return
	}
	h.base.send(ctx, u.VKID, fmt.Sprintf("История чата \"%s\" очищена.", title), keyboards.MainMenu())
}
