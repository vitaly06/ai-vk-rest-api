...
func (h *UserHandler) handleMainState(ctx context.Context, u *models.User, msg object.MessagesMessage, cmd, text string) {
	...
	case "user_clear_mirror":
		h.clearUserDialogHistory(ctx, u, models.DialogMain, "Р—РµСЂРєР°Р»Рѕ")
	case "user_clear_map":
		h.clearUserDialogHistory(ctx, u, models.DialogMap, "РљР°СЂС‚Р°")
	case "support":
...
func (h *UserHandler) clearUserDialogHistory(ctx context.Context, u *models.User, dtype models.DialogType, title string) {
	dialog, err := h.dialogRepo.GetOrCreateDialog(ctx, u.ID, dtype)
	if err != nil {
		slog.Error("user clear dialog get/create", "err", err, "dialog_type", dtype, "user_id", u.ID)
		h.base.send(ctx, u.VKID, "РќРµ СѓРґР°Р»РѕСЃСЊ РѕС‚РєСЂС‹С‚СЊ РёСЃС‚РѕСЂРёСЋ РґР»СЏ РѕС‡РёСЃС‚РєРё.", keyboards.MainMenu())
		return
	}
	if err := h.dialogRepo.ClearHistory(ctx, dialog.ID); err != nil {
		slog.Error("user clear dialog history", "err", err, "dialog_id", dialog.ID, "user_id", u.ID)
		h.base.send(ctx, u.VKID, "РќРµ СѓРґР°Р»РѕСЃСЊ РѕС‡РёСЃС‚РёС‚СЊ РёСЃС‚РѕСЂРёСЋ.", keyboards.MainMenu())
		return
	}
	h.base.send(ctx, u.VKID, fmt.Sprintf("РСЃС‚РѕСЂРёСЏ С‡Р°С‚Р° \"%s\" РѕС‡РёС‰РµРЅР°.", title), keyboards.MainMenu())
}
